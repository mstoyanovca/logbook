...
export class QsoFactory {
    ...
    static createQsos = (): QSO[] => {
        const qso1 = new QSO(1, QsoFactory.date1, QsoFactory.time1, 'LZ2VA', '144.000', 'FM', 'Kiro', 'Karlovo');
        const qso2 = new QSO(2, QsoFactory.date2, QsoFactory.time2, 'LZ1YY', '3.560', 'SSB', 'Pesho', 'Shumen');
        const qso3 = new QSO(3, QsoFactory.date3, QsoFactory.time3, 'LZ2AB', '446.110', 'RTTY', 'Bochito', 'Ruse');
        const qso4 = new QSO(4, QsoFactory.date4, QsoFactory.time4, 'LZ4MN', '29.125', 'FM', 'Krasyo', 'Smolyan');
        ...
        for (let i = 0; i < 20; i++) {
            ...
            const qso = new QSO(5 + i, date, time, 'LZ2KVV', '466.654', 'RTTY', 'Ivan', 'Varna');
